...
#include "MSFS\Render\fontstash.h"
...
struct NVGpathCacheData {
  NVGpathCache* cache;
  float* commands;
  int ncommands = 0;
  int ccommands = NVG_INIT_COMMANDS_SIZE;
...
struct NVGcontext {
  ...
  // Paths
  NVGpathCacheData pathData[NVG_MAX_PATHS];
  int activePath;
  // NVGpathCache* cache;
  // float* commands;
  // int ccommands;
...
MSFS_CALLBACK NVGcontext* nvgCreateInternal(NVGparams* params) {
  ...
  // ctx->params = *params;
  ctx->params.renderCreate = fsRenderCreate;
  ctx->params.renderCreateTexture = fsRenderCreateTexture;
  ctx->params.renderDeleteTexture = fsRenderDeleteTexture;
  ctx->params.renderUpdateTexture = fsRenderUpdateTexture;
  ctx->params.renderGetTextureSize = fsRenderGetTextureSize;
  ctx->params.renderViewport = fsRenderViewport;
  ctx->params.renderCancel = fsRenderCancel;
  ctx->params.renderFlush = fsRenderFlush;
  ctx->params.renderFill = fsRenderFill;
  ctx->params.renderStroke = fsRenderStroke;
  ctx->params.renderTriangles = fsRenderTriangles;
  ctx->params.renderClearStencil = fsRenderClearStencil;
  ctx->params.renderDelete = fsRenderDelete;
  ctx->params.userPtr = params->userPtr;
  ctx->params.edgeAntiAlias = params->edgeAntiAlias;

  for (i = 0; i < NVG_MAX_FONTIMAGES; i++)
    ctx->fontImages[i] = 0;

  ctx->activePath = 0;
  for (int p = 0; p < NVG_MAX_PATHS; ++p) {
    ctx->pathData[p].commands = (float*)malloc(sizeof(float) * NVG_INIT_COMMANDS_SIZE);
    if (!ctx->pathData[p].commands)
      goto error;
    ctx->pathData[p].ncommands = 0;
    ctx->pathData[p].ccommands = NVG_INIT_COMMANDS_SIZE;

    ctx->pathData[p].cache = nvg__allocPathCache();
    if (ctx->pathData[p].cache == NULL)
      goto error;
  }
  nvgSave(ctx);
  nvgReset(ctx);
  ...
  if (ctx->fontImages[0] == 0)
    goto error;
  ctx->pathData[ctx->activePath].fontImageIdx = 0;
...
MSFS_CALLBACK NVGparams* nvgInternalParams(NVGcontext* ctx) {
  return &ctx->params;
}

MSFS_CALLBACK void nvgDeleteInternal(NVGcontext* ctx) {
  int i;
  if (ctx == NULL)
    return;

  for (int p = 0; p < NVG_MAX_PATHS; ++p) {
    if (ctx->pathData[p].commands != NULL)
      free(ctx->pathData[p].commands);
    if (ctx->pathData[p].cache != NULL)
      nvg__deletePathCache(ctx->pathData[p].cache);
  }
  if (ctx->fs)
...
void nvgEndFrame(NVGcontext* ctx) {
  ...
  if (ctx->pathData[ctx->activePath].fontImageIdx != 0) {
    int fontImage = ctx->fontImages[ctx->pathData[ctx->activePath].fontImageIdx];
    int i, j, iw, ih;
    // delete images that smaller than current one
    if (fontImage == 0)
      return;
    nvgImageSize(ctx, fontImage, &iw, &ih);
    for (i = j = 0; i < ctx->pathData[ctx->activePath].fontImageIdx; i++) {
      if (ctx->fontImages[i] != 0) {
        int nw, nh;
...
int nvgCreateImageRGBA(NVGcontext* ctx, int w, int h, int imageFlags, const unsigned char* data) {
  return ctx->params.renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_RGBA, w, h, imageFlags, data, "NanoVG tex RGBA");
}
...
void nvgResetStencil(NVGcontext* ctx) {
  ctx->params.renderClearStencil(ctx->params.userPtr);
}
...
static void nvg__appendCommands(NVGcontext* ctx, float* vals, int nvals) {
  ...
  if (ctx->pathData[ctx->activePath].ncommands + nvals > ctx->pathData[ctx->activePath].ccommands) {
    float* commands;
    int ccommands = ctx->pathData[ctx->activePath].ncommands + nvals + ctx->pathData[ctx->activePath].ccommands / 2;
    commands = (float*)realloc(ctx->pathData[ctx->activePath].commands, sizeof(float) * ccommands);
    if (commands == NULL)
      return;
    ctx->pathData[ctx->activePath].commands = commands;
    ctx->pathData[ctx->activePath].ccommands = ccommands;
  }

  if ((int)vals[0] != NVG_CLOSE && (int)vals[0] != NVG_WINDING) {
    ctx->pathData[ctx->activePath].commandx = vals[nvals - 2];
    ctx->pathData[ctx->activePath].commandy = vals[nvals - 1];
  }
  ...
  memcpy(&ctx->pathData[ctx->activePath].commands[ctx->pathData[ctx->activePath].ncommands], vals, nvals * sizeof(float));

  ctx->pathData[ctx->activePath].ncommands += nvals;
}

static void nvg__clearPathCache(NVGcontext* ctx) {
  ctx->pathData[ctx->activePath].cache->npoints = 0;
  ctx->pathData[ctx->activePath].cache->npaths = 0;
}

static NVGpath* nvg__lastPath(NVGcontext* ctx) {
  if (ctx->pathData[ctx->activePath].cache->npaths > 0)
    return &ctx->pathData[ctx->activePath].cache->paths[ctx->pathData[ctx->activePath].cache->npaths - 1];
  return NULL;
}

static void nvg__addPath(NVGcontext* ctx) {
  NVGpath* path;
  if (ctx->pathData[ctx->activePath].cache->npaths + 1 > ctx->pathData[ctx->activePath].cache->cpaths) {
    NVGpath* paths;
    int cpaths = ctx->pathData[ctx->activePath].cache->npaths + 1 + ctx->pathData[ctx->activePath].cache->cpaths / 2;
    paths = (NVGpath*)realloc(ctx->pathData[ctx->activePath].cache->paths, sizeof(NVGpath) * cpaths);
    if (paths == NULL)
      return;
    ctx->pathData[ctx->activePath].cache->paths = paths;
    ctx->pathData[ctx->activePath].cache->cpaths = cpaths;
  }
  path = &ctx->pathData[ctx->activePath].cache->paths[ctx->pathData[ctx->activePath].cache->npaths];
  memset(path, 0, sizeof(*path));
  path->first = ctx->pathData[ctx->activePath].cache->npoints;
  path->winding = NVG_CCW;

  ctx->pathData[ctx->activePath].cache->npaths++;
}

static NVGpoint* nvg__lastPoint(NVGcontext* ctx) {
  if (ctx->pathData[ctx->activePath].cache->npoints > 0)
    return &ctx->pathData[ctx->activePath].cache->points[ctx->pathData[ctx->activePath].cache->npoints - 1];
  return NULL;
}
...
static void nvg__addPoint(NVGcontext* ctx, float x, float y, int flags) {
  ...
  if (path->count > 0 && ctx->pathData[ctx->activePath].cache->npoints > 0) {
  ...
  if (ctx->pathData[ctx->activePath].cache->npoints + 1 > ctx->pathData[ctx->activePath].cache->cpoints) {
    NVGpoint* points;
    int cpoints = ctx->pathData[ctx->activePath].cache->npoints + 1 + ctx->pathData[ctx->activePath].cache->cpoints / 2;
    points = (NVGpoint*)realloc(ctx->pathData[ctx->activePath].cache->points, sizeof(NVGpoint) * cpoints);
    if (points == NULL)
      return;
    ctx->pathData[ctx->activePath].cache->points = points;
    ctx->pathData[ctx->activePath].cache->cpoints = cpoints;
  }

  pt = &ctx->pathData[ctx->activePath].cache->points[ctx->pathData[ctx->activePath].cache->npoints];
  memset(pt, 0, sizeof(*pt));
  pt->x = x;
  pt->y = y;
  pt->flags = (unsigned char)flags;

  ctx->pathData[ctx->activePath].cache->npoints++;
  path->count++;
}
...
static NVGvertex* nvg__allocTempVerts(NVGcontext* ctx, int nverts) {
  if (nverts > ctx->pathData[ctx->activePath].cache->cverts) {
    NVGvertex* verts;
    int cverts = (nverts + 0xff) & ~0xff;  // Round up to prevent allocations when things change just slightly.
    verts = (NVGvertex*)realloc(ctx->pathData[ctx->activePath].cache->verts, sizeof(NVGvertex) * cverts);
    if (verts == NULL)
      return NULL;
    ctx->pathData[ctx->activePath].cache->verts = verts;
    ctx->pathData[ctx->activePath].cache->cverts = cverts;
  }

  return ctx->pathData[ctx->activePath].cache->verts;
}
...
static void
nvg__tesselateBezier(NVGcontext* ctx, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, int level, int type) {
...
static void nvg__flattenPaths(NVGcontext* ctx) {
  NVGpathCache* cache = ctx->pathData[ctx->activePath].cache;
  //	NVGstate* state = nvg__getState(ctx);
  NVGpoint* last;
  NVGpoint* p0;
  NVGpoint* p1;
  ...
  // Flatten
  i = 0;
  while (i < ctx->pathData[ctx->activePath].ncommands) {
    int cmd = (int)ctx->pathData[ctx->activePath].commands[i];
    switch (cmd) {
      case NVG_MOVETO:
        nvg__addPath(ctx);
        p = &ctx->pathData[ctx->activePath].commands[i + 1];
      ...
      case NVG_BEZIERTO:
        ...
        if (last != NULL) {
          cp1 = &ctx->pathData[ctx->activePath].commands[i + 1];
          cp2 = &ctx->pathData[ctx->activePath].commands[i + 3];
          p = &ctx->pathData[ctx->activePath].commands[i + 5];
      ...
      case NVG_CLOSE:
        ...
        break;
      case NVG_WINDING:
        nvg__pathWinding(ctx, (int)ctx->pathData[ctx->activePath].commands[i + 1]);
        i += 2;
        break;
      default:
        i++;
...
static NVGvertex*
nvg__roundJoin(NVGvertex* dst, NVGpoint* p0, NVGpoint* p1, float lw, float rw, float lu, float ru, int ncap, float fringe) {
  int i, n;
  float dlx0 = p0->dy;
  float dly0 = -p0->dx;
  float dlx1 = p1->dy;
...
void nvgBeginPath(NVGcontext* ctx) {
  ctx->pathData[ctx->activePath].ncommands = 0;
...
void nvgQuadTo(NVGcontext* ctx, float cx, float cy, float x, float y) {
  float x0 = ctx->pathData[ctx->activePath].commandx;
  float y0 = ctx->pathData[ctx->activePath].commandy;
...
void nvgArcTo(NVGcontext* ctx, float x1, float y1, float x2, float y2, float radius) {
  ...
  if (ctx->pathData[ctx->activePath].ncommands == 0) {
    return;
  }
  ...
  nvgEllipticalArc(ctx, cx, cy, radius, radius, a0, a1, dir);
}
...
void nvgEllipticalArc(NVGcontext* ctx, float cx, float cy, float rx, float ry, float a0, float a1, int dir) {
  ...
  int i, ndivs, nvals;
  int move = ctx->pathData[ctx->activePath].ncommands > 0 ? NVG_LINETO : NVG_MOVETO;
...
void nvgArc(NVGcontext* ctx, float cx, float cy, float r, float a0, float a1, int dir) {
  nvgEllipticalArc(ctx, cx, cy, r, r, a0, a1, dir);
}
...
void nvgDebugDumpPathCache(NVGcontext* ctx) {
  ...
  for (i = 0; i < ctx->pathData[ctx->activePath].cache->npaths; i++) {
    path = &ctx->pathData[ctx->activePath].cache->paths[i];
    printf(" - Path %d\n", i);
    if (path->nfill) {
      printf("   - fill: %d\n", path->nfill);
...
void nvgFill(NVGcontext* ctx) {
  ...
  ctx->params.renderFill(ctx->params.userPtr, &fillPaint, state->compositeOperation, &state->scissor, ctx->fringeWidth,
                         ctx->pathData[ctx->activePath].cache->bounds, ctx->pathData[ctx->activePath].cache->paths,
                         ctx->pathData[ctx->activePath].cache->npaths);
...
void nvgStroke(NVGcontext* ctx) {
  ...
  ctx->params.renderStroke(ctx->params.userPtr, &strokePaint, state->compositeOperation, &state->scissor, ctx->fringeWidth, strokeWidth,
                           ctx->pathData[ctx->activePath].cache->paths, ctx->pathData[ctx->activePath].cache->npaths);
...
static int nvg__allocTextAtlas(NVGcontext* ctx) {
  ...
  if (ctx->pathData[ctx->activePath].fontImageIdx >= NVG_MAX_FONTIMAGES - 1)
    return 0;
  // if next fontImage already have a texture
  if (ctx->fontImages[ctx->pathData[ctx->activePath].fontImageIdx + 1] != 0)
    nvgImageSize(ctx, ctx->fontImages[ctx->pathData[ctx->activePath].fontImageIdx + 1], &iw, &ih);
  else {  // calculate the new font image size and create it.
    nvgImageSize(ctx, ctx->fontImages[ctx->pathData[ctx->activePath].fontImageIdx], &iw, &ih);
    if (iw > ih)
      ih *= 2;
    else
      iw *= 2;
    ...
    ctx->fontImages[ctx->pathData[ctx->activePath].fontImageIdx + 1] =
        ctx->params.renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_ALPHA, iw, ih, 0, NULL, "NanoVG text atlas");
  }
  ++ctx->pathData[ctx->activePath].fontImageIdx;
...
static void nvg__renderText(NVGcontext* ctx, NVGvertex* verts, int nverts) {
  ...
  // Render triangles.
  paint.image = ctx->fontImages[ctx->pathData[ctx->activePath].fontImageIdx];
  ...
  while (fonsTextIterNext(ctx->fs, &iter, &q)) {
    ...
    switch (iter.codepoint) {
      ...
      case 0x0085:  // NEL
        ...
        break;
      default:
        if ((iter.codepoint >= 0x4E00 && iter.codepoint <= 0x9FFF) || (iter.codepoint >= 0x3000 && iter.codepoint <= 0x30FF) ||
            (iter.codepoint >= 0xFF00 && iter.codepoint <= 0xFFEF) || (iter.codepoint >= 0x1100 && iter.codepoint <= 0x11FF) ||
            (iter.codepoint >= 0x3130 && iter.codepoint <= 0x318F) || (iter.codepoint >= 0xAC00 && iter.codepoint <= 0xD7AF))
          type = NVG_CJK_CHAR;
        else
          type = NVG_CHAR;
        break;
