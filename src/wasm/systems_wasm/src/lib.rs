...
impl<'a, 'b> MsfsSimulationBuilder<'a, 'b> {
    ...
    pub fn build<T: Aircraft, U: FnOnce(&mut InitContext) -> T>(
        ...
        Ok((
            simulation,
            MsfsHandler::new(registry, self.aspects, self.sim_connect)?,
        ))
    }
    ...
    //Conects the rust code with the systems.cfg file
    ...
    pub fn provides_aircraft_variable( mut self, name: &str, units: &str, index: usize, ) -> Result<Self, Box<dyn Error>> {
